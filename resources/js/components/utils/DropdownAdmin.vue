    ...
    <div>
        <div class="relative z-10">
            ...
            <div v-if="show === true" class="absolute left-2 mt-2 w-44 rounded-md bg-gray-100 py-2 shadow-xl">
                <a v-if="this.role === 'administrator'"
                    @click="goDashboard"
                ...
                <a v-if="this.role === 'administrator' || this.role === 'stock_assistant'"
                    @click="goCategories"
                    class="block cursor-pointer px-4 py-2 text-xs font-extrabold uppercase text-orangePantone hover:bg-orangePantone hover:text-white"
                >
                    {{ __('general.sidebar.categories') }}
                </a>
                <a  v-if="this.role === 'administrator' || this.role === 'stock_assistant' || this.role === 'financial_assistant'"
                    @click="goProducts"
                    class="block cursor-pointer px-4 py-2 text-xs font-extrabold uppercase text-orangePantone hover:bg-orangePantone hover:text-white"
                >
                    {{ __('general.sidebar.products') }}
                </a>
                <a v-if="this.role === 'administrator' || this.role === 'financial_assistant' || this.role === 'logistic_assistant'"
                    @click="goOrders"
                    class="block cursor-pointer px-4 py-2 text-xs font-extrabold uppercase text-orangePantone hover:bg-orangePantone hover:text-white"
                >
                ...
                <a v-if="this.role === 'administrator' || this.role === 'financial_assistant' || this.role === 'marketing_assistant'"
                    @click="goReports"
                    class="block cursor-pointer px-4 py-2 text-xs font-extrabold uppercase text-orangePantone hover:bg-orangePantone hover:text-white"
                >
